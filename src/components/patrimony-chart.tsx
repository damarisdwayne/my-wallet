...
export const PatrimonyChart = memo(({ history, currentValue, currentMonth }: Props) => {
  ...
  // merge stored history with current month's live value
    ...
    <div className="space-y-4">
      ...
      <div className="relative w-full select-none">
        ...
        >
          ...
          {/* dots (only visible on hover or for first/last) */}
          {[0, n - 1].map((i) => (
            <circle
              key={i}
              cx={pts[i].x}
              cy={pts[i].y}
              r="4"
              fill={positive ? successColor : destructiveColor}
              opacity="0.7"
            />
          ))}
